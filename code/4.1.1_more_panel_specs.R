...
for (it in unique(cost_panel$incident_type)){
  for (ds in unique(cost_panel$data_series)){
    for (at in unique(cost_panel$assistance_type)){
      if ((nrow(subset(select(cost_panel, fips_code, date, zhvi, data_series, incident_type, assistance_type, dis_lag_0.25, dis_lag_0.5_e, dis_lag_1_e), incident_type == it & data_series == ds & assistance_type == at)) > 0) & (sum(select(subset(select(dummy_occ_type_panel, fips_code, date, zhvi, data_series, incident_type, assistance_type, dis_lag_0.25, dis_lag_0.5_e, dis_lag_1_e), incident_type == it & data_series == ds & assistance_type == at), starts_with("dis_")), na.rm = TRUE) > 0)){
...
for (it in unique(cost_panel$incident_type)){
  for (ds in unique(cost_panel$data_series)){
    for (at in unique(cost_panel$assistance_type)){
      if ((nrow(subset(select(cost_panel, fips_code, date, zhvi, data_series, incident_type, assistance_type, gdp_value, unemployment_rate, avg_wkly_wage, dis_lag_0.25), incident_type == it & data_series == ds & assistance_type == at)) > 0) & (sum(select(subset(select(dummy_occ_type_panel, fips_code, date, zhvi, data_series, incident_type, assistance_type, gdp_value, unemployment_rate, avg_wkly_wage, dis_lag_0.25), incident_type == it & data_series == ds & assistance_type == at), starts_with("dis_")), na.rm = TRUE) > 0)){
...
for (it in unique(cost_panel$incident_type)){
  for (ds in unique(cost_panel$data_series)){
    for (at in unique(cost_panel$assistance_type)){
      if ((nrow(subset(select(cost_panel, fips_code, date, zhvi, data_series, incident_type, assistance_type, gdp_value, unemployment_rate, avg_wkly_wage, dis_lag_0.25, dis_lag_0.5_e, dis_lag_1_e), incident_type == it & data_series == ds & assistance_type == at)) > 0) & (sum(select(subset(select(dummy_occ_type_panel, fips_code, date, zhvi, data_series, incident_type, assistance_type, gdp_value, unemployment_rate, avg_wkly_wage, dis_lag_0.25, dis_lag_0.5_e, dis_lag_1_e), incident_type == it & data_series == ds & assistance_type == at), starts_with("dis_")), na.rm = TRUE) > 0)){
