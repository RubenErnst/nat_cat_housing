...
zhvi_by_data_series_line <- aggregate(zhvi ~ data_series + lubridate::year(date), zillow_county, median, na.rm = T) |>
  ...
  scale_y_continuous(labels = function(x){format(x, big.mark = "'", scientific = FALSE)}, name = "Zillow Home Value Index", limits = c(0, 450000)) +
...
# TODO FEMA disaster maps by incident_category